-- Email redesign: every outbound email now shares one visual shell — the star
-- mark up top, a clean system sans stack instead of monospace, a bulletproof
-- centered 480px column (tables, so Outlook honors the width), quoted user
-- text as a hairline blockquote, and a divider-topped footer. Logic is
-- untouched: throttles, advisory lock, idempotency guards, log-before-send,
-- and subject sanitization are carried over verbatim from migration22.
-- Idempotent.
...
returns text language sql immutable as $$
  select
    ...
    || '<img src="https://bvpruvuenqdwilysgojy.supabase.co/storage/v1/object/public/assets/icon-192.png" width="28" height="28" alt="Favorites" style="display:block;margin:0 0 28px;border:0">'
    || p_content
...
begin
  ...
  return format(
    ...
    || 'Favorites · <a href="https://myfavoriteapp.com/%s" style="color:#a1a1aa">your page</a>'
